...
form.addEventListener("submit", async (e) => {
  ...
  if (!form.checkValidity()) {
    show(error, "Перевірте, будь ласка, обов'язкові поля, помічені зірочкою *");
    return;
  }
  ...
  if (!payload.phone || payload.phone.replace(/[^\d]/g,'').length < 7) {
    show(error, "Вкажіть коректний номер телефону.");
    return;
  }
  ...
  try {
    ...
    if (ok && body && body.ok) {
      show(success, "Дякуємо! Заявку надіслано. Ми зв'яжемося з вами найближчим часом.");
      form.reset();
      ...
      const msg = (body && (body.error || body.message)) || `Помилка від сервера (${status}). Спробуємо відправити лист.`;
      show(error, msg);
    }
  } catch (err) {
