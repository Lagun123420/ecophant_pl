...
form.addEventListener("submit", async (e) => {
  ...
  if (!form.checkValidity()) {
    show(error, "Sprawdź proszę pola obowiązkowe oznaczone gwiazdką *");
    return;
  }
  ...
  if (!payload.phone || payload.phone.replace(/[^\d]/g,'').length < 7) {
    show(error, "Podaj poprawny numer telefonu.");
    return;
  }
  ...
  try {
    ...
    if (ok && body && body.ok) {
      show(success, "Dziękujemy! Zgłoszenie zostało wysłane. Skontaktujemy się z Tobą wkrótce.");
      form.reset();
      ...
      const msg = (body && (body.error || body.message)) || `Błąd serwera (${status}).`;
      show(error, msg);
    }
  } catch (err) {
